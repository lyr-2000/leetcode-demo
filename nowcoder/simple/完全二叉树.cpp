...
void printt(const string &prefix, TreeNode *node, bool isLeft=false) { if (node != nullptr) { cout << prefix; cout << (isLeft ? "├──" : "└──"); cout << node->val << endl; printt(prefix + (isLeft ? "│ " : " "), node->left, true); printt(prefix + (isLeft ? "│ " : " "), node->right, false); } }
void print(ListNode *h) { while(h) { print(h->val); h = h->next; } println("");}
void print(TreeNode *node) { printf("\n");printt("",node,false);}
...
int main()
{
    ...
    cout << "result:  " << k.isCompleteTree(maketree(arr)) << endl;
    return 0;
}
